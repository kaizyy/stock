...
function enhance(){
  document.querySelectorAll('.order-card').forEach(card=>{
    ...
    const type=status?.dataset.orderType||del?.dataset.orderType||'';const wrap=document.createElement('div');wrap.className='doc-v2-actions';const links=[];
...
const style=document.createElement('style');style.textContent='.doc-v2-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:10px}.doc-v2-actions .button{padding:7px 9px;text-decoration:none}@media(max-width:600px){.doc-v2-actions{display:grid;grid-template-columns:1fr 1fr}.doc-v2-actions .button{text-align:center}}';document.head.appendChild(style);
let pending=false;const run=()=>{if(pending)return;pending=true;requestAnimationFrame(()=>{pending=false;enhance()})};
window.addEventListener('load',()=>{enhance();const root=document.getElementById('orders');if(root)new MutationObserver(run).observe(root,{childList:true,subtree:true})});document.addEventListener('stockroom:refresh',()=>setTimeout(enhance,100));
})();