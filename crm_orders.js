(() => {
  ...
  function installUI(){
    ...
    relations.innerHTML=`<div class="section-head"><div><p class="eyebrow">Contacten</p><h2>Leveranciers & klanten</h2></div></div><div id="crmMessage"></div><div class="crm-grid"><article class="panel" id="supplierPanel"><div class="panel-head"><div><p class="eyebrow">Inkoop</p><h3>Leveranciers</h3></div></div><form id="supplierForm" class="crm-form"><input type="hidden" name="id"><input name="name" placeholder="Bedrijfsnaam" required><input name="contact_name" placeholder="Contactpersoon"><input name="email" type="email" placeholder="E-mail"><input name="phone" placeholder="Telefoon"><input name="address" placeholder="Adres"><input name="notes" placeholder="Notities"><button class="button primary" type="submit">Leverancier opslaan</button></form><div id="supplierList" class="crm-list"></div></article><article class="panel" id="customerPanel"><div class="panel-head"><div><p class="eyebrow">Verkoop</p><h3>Klanten</h3></div></div><form id="customerForm" class="crm-form"><input type="hidden" name="id"><input name="name" placeholder="Klant / bedrijf" required><input name="contact_name" placeholder="Contactpersoon"><input name="email" type="email" placeholder="E-mail"><input name="phone" placeholder="Telefoon"><input name="address" placeholder="Adres"><input name="notes" placeholder="Notities"><button class="button primary" type="submit">Klant opslaan</button></form><div id="customerList" class="crm-list"></div></article></div>`;
  ...
  function renderRelations(kind,rows){const target=document.getElementById(kind==='supplier'?'supplierList':'customerList');const canWrite=roleCan(me.stockroom.role,kind,true);target.innerHTML=rows.length?rows.map(r=>`<div class="crm-card"><div class="crm-card-head"><div><strong>${esc(r.name)}</strong><small>${esc(r.contact_name||'')}${r.email?` · ${esc(r.email)}`:''}${r.phone?` · ${esc(r.phone)}`:''}</small>${r.address?`<small>${esc(r.address)}</small>`:''}</div>${canWrite?`<button class="crm-edit" type="button" data-edit-relation="${kind}" data-id="${esc(r.id)}">Bewerken</button>`:''}</div></div>`).join(''):'<small>Nog geen relaties.</small>'}
