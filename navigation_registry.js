(()=>{
  const entries=[
    ...
    {id:'settings',label:'Instellingen',area:'management',view:'settings',trigger:'[data-view="settings"]',subviews:['account-organization','documents-invoices','users-rights','inventory-settings','notifications-settings','security-integrations','backup-restore','subscription'],features:['account','organization','documents','members-roles','inventory-settings','notifications-settings','security-integrations','audit-log','backup-restore','billing']},
  ...
  function copy(entry){return {...entry,features:[...entry.features],subviews:[...(entry.subviews||[])]}}
  function all(){return entries.map(copy)}
  function get(id){const entry=byId.get(id);return entry?copy(entry):null}
