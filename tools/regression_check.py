...
def check_web_feature_parity() -> None:
    ...
    require(index, ['id="overview"', 'id="inventory"', 'id="incoming"', 'id="outgoing"', 'id="transactionDialog"', 'id="archiveDialog"', 'id="quickAddBtn"', 'id="inventoryBuyValue"', 'id="revenueValue"', 'id="outstandingValue"', 'id="outstandingOverdueValue"', 'id="expectedValue"', 'id="expectedPaidValue"', 'id="expectedUnpaidValue"', 'id="cashflowBalanceValue"', 'id="cashflowBufferValue"', 'id="stockChart"', 'id="revenueChart"', 'id="forecastTable"', 'id="forecastSummary"', 'inventory_forecast.js?v=', 'app.js?v=', 'styles.css?v='], "Webdashboard, actueel kasstroomsaldo en cacheverversing")
    ...
    require(read("profit_reporting.py"), ["operating_expenses", "costOfGoods", "grossProfit", "netProfit", "estimatedCost", "bySupplier", "marges-per-artikel.csv", "linked_keys", "Handmatig", "transactionType"], "Winst-en-verlies en margeanalyse met losse transacties")
    require(read("profit_reporting_ui.js"), ["analyticsContent", "profitReportPanel", "/api/profit-report", "Nettoresultaat", "Marge per artikel", "Marge per klant", "Marge per leverancier", "expenseForm", "reliableLoad", "MutationObserver"], "Betrouwbaar ladende resultaatrapportage onder Analytics")
    require(read("cashflow_forecast.py"), ["cashflow_settings", "minimum_buffer", "sales_invoice", "purchase_invoice", "payment_batch", "purchase_order", "recurring_costs", "manual_sale", "manual_purchase", "orderId", "100% van alle openstaande ontvangsten", "conservative", "optimistic"], "Kasstroomprognose 30/60/90 met losse transacties en 100%-verwachting")
    require(read("cashflow_forecast_ui.js"), ["analyticsContent", "cashflowPanel", "/api/cashflow-forecast", "Saldo na", "Conservatief", "Optimistisch", "data-cash-scenario", "Losse uitgaande transactie", "Losse inkomende transactie", "cashflowBalanceValue", "below-buffer", "reliableLoad", "loadPromise", "MutationObserver"], "Kasstroomsaldo op dashboard en betrouwbaar ladende prognose")
    require(read("analytics_dashboard.js"), ["cashflowPanel", "profitReportPanel", "taxReportPanel", "analyticsContent", "stockroom:analytics-ready"], "Financiële rapporten onder Analytics")
    ...
    for filename in ("cashflow_forecast_ui.js", "profit_reporting_ui.js", "tax_reporting_ui.js"):
        require(read(filename), ["view==='analytics'", "stockroom:analytics-ready"], f"Analytics-verversing in {filename}")
    ...
    require(read("navigation_registry.js"), ["StockroomNavigationRegistry", "resolveFeature", "auditDom", "stockroom:navigation-audit", "overview", "analytics", "inventory", "warehouse", "incoming", "outgoing", "relations", "orders", "quotes", "finance", "settings", "notifications", "platform-admin", "members", "sales-invoices", "debtors", "purchase-invoices", "payment-batches", "bank-reconciliation", "account-organization", "documents-invoices", "users-rights", "backup-restore", "subscription"], "Centraal navigatieregister met financiële en instellingen-subnavigatie")
    require(read("site_ux.js"), ["Account & organisatie", "Documenten & facturen", "Gebruikers & rechten", "Voorraadinstellingen", "Meldingen", "Beveiliging & koppelingen", "Back-up & herstel", "Abonnement", "settingsBackupPanel", "data-open-platform-backup"], "Compacte instellingen-navigatie")
    require(index, ["navigation_registry.js?v="], "Navigatieregister cacheverversing")
    require(dockerfile, ["navigation_registry.js"], "Navigatieregister in productie")
    require(read("navigation_subviews.js"), ["customers", "suppliers", "sales-orders", "purchase-orders", "customerPanel", "supplierPanel", "salesPanel", "purchasePanel", "crm-split-source", "crm-subview-hidden", "max-width:900px"], "Gesplitste relatie- en ordernavigatie")
    require(read("navigation_registry.js"), ["subviews:['customers','suppliers']", "subviews:['sales-orders','purchase-orders']"], "CRM-subpagina's in centraal register")
    require(index, ["navigation_subviews.js?v="], "CRM-navigatie cacheverversing")
    require(dockerfile, ["navigation_subviews.js"], "CRM-navigatie in productie")
    require(read("finance_navigation.js"), ["sales-invoices", "debtors", "purchase-invoices", "payment-batches", "bank-reconciliation", "Verkoopfacturen", "Debiteuren", "Inkoopfacturen", "Betaalvoorstellen", "Bankmutaties", "finance-split-source", "finance-subview-hidden", "data-debtor-open", "max-width:900px"], "Gesplitste financiële navigatie")
    require(read("navigation_registry.js"), ["subviews:['sales-invoices','debtors','purchase-invoices','payment-batches','bank-reconciliation']"], "Financiële subpagina's in centraal register")
    require(index, ["finance_navigation.js?v="], "Financiële navigatie cacheverversing")
    require(dockerfile, ["finance_navigation.js"], "Financiële navigatie in productie")
    require(read("debtor_management.py"), ["debtor_promises", "debtor_blocks", "debtor_dunning_history", "0_30", "31_60", "61_90", "90_plus", "def save_promise", "def set_block", "def send_dunning", "/api/debtors"], "Debiteurenbeheer, betaalafspraken en aanmaningen")
    require(read("debtor_management_ui.js"), ["debtorPanel", "90+ dagen", "Betaalafspraak", "Klant blokkeren", "data-debtor-alert", "/api/debtors/dunning"], "Debiteurenbediening en dashboardmelding")
    require(dockerfile, ["debtor_management.py", "debtor_management_ui.js", "debtor_management.install"], "Debiteurenbeheer in productie")
