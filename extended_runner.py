...
class ExtendedHandler(app_runner.AppHandler):
    ...
    def do_GET(self):
        ...
        if path=="/api/purchase-invoice-policy":
            s=self.require_session(api=True)
            if s:self.send_json(200,purchase_invoices.policy(s['stockroom_id']))
            return
        if path=="/api/purchase-invoices":
            s=self.require_session(api=True)
            if not s:return
            if not orders.allowed(s['role'],'read_purchase'):self.send_json(403,{"error":"Geen rechten."});return
            self.send_json(200,{"invoices":purchase_invoices.rows(s['stockroom_id'])});return
        if path=="/api/purchase-invoices/document":
            s=self.require_session(api=True)
            if not s:return
            try:
        ...
        if path=="/api/profit-report":
            s=self.require_session(api=True)
            if s:
                query=parse_qs(parsed.query);self.send_json(200,profit_reporting.report(s['stockroom_id'],query.get('year',[date.today().year])[0],query.get('mode',['month'])[0],query.get('value',[date.today().month])[0]))
            return
        if path=="/api/cashflow-forecast":
            s=self.require_session(api=True)
        ...
        if path=="/api/profit-report/export":
            ...
            if not s:return
            try:
                query=parse_qs(parsed.query);data,name=profit_reporting.export(s,query.get('year',[date.today().year])[0],query.get('mode',['month'])[0],query.get('value',[date.today().month])[0]);self.send_response(200);self.send_header('Content-Type','application/zip');self.send_header('Content-Disposition',f'attachment; filename="{name}"');self.send_header('Content-Length',str(len(data)));self.end_headers();self.wfile.write(data)
            except (ValueError,PermissionError) as exc:self.send_json(400,{"error":str(exc)})
            return
